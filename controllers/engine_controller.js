import EngineModel from '../models/engine_model.js'; //importing the model

//creating the route logic and exporting it immediately so it can be used by the route as a function
export const getEngineList = async (req, res) => {
    try {
        const list = await EngineModel.find(); //get all the documents without filter
