import TransmissionModel from '../models/transmission_model.js'; //importing the model

//creating the route logic and exporting it immediately so it can be used by the route as a function
export const getTransmissionList = async (req, res) => {
    try {
        const list = await TransmissionModel.find(); //get all the documents without filter
