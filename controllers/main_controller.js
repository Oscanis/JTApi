import MainModel from '../models/main_model.js'; //importing the model

//creating the route logic and exporting it immediately so it can be used by the route as a function
export const getMainList = async (req, res) => {
    try {
        const list = await MainModel.find(); //get all the documents without filter
