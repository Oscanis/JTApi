import DriveModel from '../models/drive_model.js'; //importing the model

//creating the route logic and exporting it immediately so it can be used by the route as a function
export const getDriveList = async (req, res) => {
    try {
        const list = await DriveModel.find(); //get all the documents without filter
