import BodyModel from '../models/body_model.js'; //importing the model

//creating the route logic and exporting it immediately so it can be used by the route as a function
export const getBodyList = async (req, res) => {
    try {
        const list = await BodyModel.find(); //get all the documents without filter
